...
app.get('/jokes/:id', (req, res)=>{
  var id = parseInt(req.params.id); //convert a route parameter from a string to an integer.
  var jokeId = jokes.find((joke)=> joke.id===id) //find is an array method that returns the first element in the array that satisfies the provided testing function.
  res.json(jokeId);
})

//3. GET a jokes by filtering on the joke type
app.get('/filter', (req, res)=>{
  var type = req.query.type; //access the value of the type query parameter from the URL.
  var jokeType = jokes.filter((joke)=> joke.jokeType===type)
  res.json(jokeType);
})
